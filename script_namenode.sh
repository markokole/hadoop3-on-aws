...
# create user mapred
sudo su -c "useradd mapred -s /bin/bash"
echo "mapred:mapred" | sudo chpasswd

# add user mapred to hadoop group
...
# create metadata directories for Namenodes and DataNodes
sudo -u hadoop mkdir -p $HADOOP_HOME/metadata/NameNode
sudo -u hadoop mkdir $HADOOP_HOME/metadata/DataNode
sudo chown hdfs:hadoop -R $HADOOP_HOME/metadata
#sudo -u hadoop chmod -R 775 $HADOOP_HOME/metadata
...
sudo -u hadoop rm -f $HADOOP_HOME/etc/hadoop/hdfs-site.xml
sudo -u hadoop echo '<configuration>
    <property>
...
# create tmp in hdfs
sudo -u hdfs $HADOOP_HOME/bin/hadoop fs -mkdir -p /tmp
